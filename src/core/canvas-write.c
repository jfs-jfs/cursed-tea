...
void _ct_cwrite_char_rec(struct CtCanvas *canvas, const CtUVector from,
  ...
  float distance1 = ct_uvec_distance((CtUVector){from.x + 1, from.y}, to);
  float distance2 = ct_uvec_distance((CtUVector){from.x, from.y + 1}, to);

  if (distance1 < distance2) {
    _ct_cwrite_char_rec(canvas, (CtUVector){from.x + 1, from.y}, to, character);
    return;
  }

  _ct_cwrite_char_rec(canvas, (CtUVector){from.x, from.y + 1}, to, character);
}
...
void ct_cwrite(struct CtCanvas *canvas, const size_t x, const size_t y,
  ...
  if (!ct_uvec_is_inside(canvas_area, start) || !ct_uvec_fits(canvas_area, end)) {
    log_error(L"Trying to write outside of canvas!");
    return;
  }
